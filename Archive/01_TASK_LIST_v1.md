...
> ⚠️ **ISSUE-01 반영:** Sprint 0을 2주(0A/0B)로 확장. DB-012~015(AI 관련), DB-016을 Sprint 0B로 이동.
> ⚠️ **ISSUE-04 반영:** DTO Draft 2단계 전략 채택 — Sprint 0A에서 DB 스키마 초안 기반 DTO Draft 병렬 작성 가능.
> ⚠️ **ISSUE-06 반영:** H 복잡도 태스크는 🔍 마커 표시 — AI 초안 생성 후 Human Review 필수.

| Sprint | 주요 태스크 ID | 비고 |
|---|---|---|
| **Sprint 0A (1주)** | DB-001~011, API-007, MOCK-001~005 | 핵심 DB 스키마 SSOT + Seed 데이터 (L/M 복잡도 17개) |
| **Sprint 0B (1주)** | DB-012~015, DB-016🔍, DB-017🔍, API-001~006, API-008🔍, MOCK-006 | AI 테이블 + DTO 확정 + Mock API (14개, DTO Draft→확정) |
| **Sprint 1 (2주)** | AUTH-C-001~005, SA-C-002🔍, SA-Q-001~002, ZUI-C-001🔍~003🔍, INT-C-001🔍~003, BULK-C-001🔍~002🔍, INFRA-001🔍~002, DEVOPS-001~003, UI-001~003, EDGE-UI-001🔍~002🔍 | Slice-1 Core |
| **Sprint 2 (2주)** | NC-C-001a~001c, NC-C-002~004, ZUI-C-004~006, PRIV-001🔍~003🔍, UI-006~008, EDGE-UI-003~005 | NC 시정 + Vision AI + 가명처리 |
| **Sprint 3 (2주)** | LEAN-C-001~004, AI-C-001~008, UI-009~012, INFRA-010~012 | Lean 진단 + AI 거버넌스 |
| **Sprint 4 (2주)** | VD-C-001~003, PRIV-004🔍~006, USE-001~003, INFRA-006🔍~009🔍, INFRA-014🔍, 전체 TEST 태스크 | 마무리·폴리싱·테스트 |

### H 복잡도 태스크 Human Review 정책 (ISSUE-06)

> 🔍 마커가 표시된 **H 복잡도 태스크**는 아키텍처 판단이 필요하여 AI 단독 수행 시 설계 오류 위험이 있습니다.
> - AI 에이전트는 **초안(Draft)** 생성까지만 담당
> - 시니어 개발자 리뷰 후 확정하는 워크플로 적용
> - 각 TASK_SPEC에 `requires_human_review: true` 플래그 명시
...
| NC 시정 (Epic 6) | **8** | NC-C-001 → 001a/b/c 분리 (+2) |
| Zero-UI (Epic 7) | 7 | |
| Lean 진단 (Epic 8) | 6 | |
| 데이터 무결성 (Epic 9) | 5 | |
