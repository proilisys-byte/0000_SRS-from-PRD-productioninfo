...
- **5개 국어 지원 구조**:
  ...
  - **지원 언어**: 한국어, 영어, 베트남어, 네팔어, 캄보디아어 *(SRS REQ-NF-PRIV-001 준수)*
...
- **동의 철회 시 처리 방안**:
  ...
  - **동의 철회 처리 SLA**: 영업일 기준 **3일 이내** 데이터 처리 중단 및 파기 절차 완료 *(SRS REQ-NF-PRIV-002 준수)*

### C-1. 정보주체 권리 행사 대응 (REQ-NF-PRIV-004 준수)

**개인정보보호법 제35조~제37조**에 명시된 열람·정정·삭제·처리정지권 요청에 대한 자동화 프로세스:

| 권리 유형 | 조치 SLA | 처리 방식 | 비고 |
|:---|:---:|:---|:---|
| **열람권** (제35조) | 10일 이내 | `GET /api/user/data-export` → JSON/CSV 형태 다운로드 API 제공 | Admin 대시보드에서 관리 |
| **정정권** (제36조) | 10일 이내 | 지원 티켓 → DPO 검토 → DB 정정 + Audit Log 기록 | Insert-only 원칙하 신규 행 추가 방식 |
| **삭제권** (제36조) | 10일 이내 | Cryptographic Erasure (DEK 파기) 적용 → 삭제 증빙 Audit Log 기록 | 감사 로그 자체는 법적 보존 예외 |
| **처리정지권** (제37조) | 10일 이내 | `user_consents` 테이블에 `PROCESSING_SUSPENDED` 상태 Insert → 해당 사용자 데이터 처리 파이프라인 즉시 중단 | 관련 API 엔드포인트 접근 차단 |

- **자동화 구현**: 설정(Settings) 메뉴 또는 관리자 대시보드에 '정보주체 권리 행사' 관리 패널 구축
- **처리 로그**: 모든 권리 행사 요청 및 처리 결과는 `rights_request_log` 테이블에 기록 (Insert-only)

### C-2. Edge 가명처리 정책 (REQ-NF-PRIV-003 준수)

- **음성 데이터**: Cloud 전송 전 Edge-side에서 화자 특성(피치 등)을 비가역적으로 변환
- **Vision AI 구동 시**: **얼굴 영역 자동 마스킹** 처리 필수 (가명정보화 원칙 준수)
- **검증 기준**: 가명처리 미적용 전송 건수 = **0건** (네트워크 프록시 레벨 검출)
