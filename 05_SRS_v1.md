...
| S-5b | Frontend/Edge App Engineer | Implementer | 웹 대시보드, Edge 디바이스 앱 개발 | Zero-UI 인터페이스 요구사항, 온라인 연동 사양 |
...
erDiagram
    ...
    TENANT ||--o{ INSERT_ONLY_AUDIT_LOG : "tracked by"
...
|:---|:---|:---|
| Admin | 시스템 설정, 사용자 관리, 리포트 최종 승인, 감사 로그 조회 | — |
| User | 데이터 입력, NC 작성, 리포트 생성 요청(조회 전용) | 승인 및 시스템 설정 불가 |

| ID | Category (범주) | Verification (검증) | Acceptance Criteria (인수/측정 기준) |
|:---|:---|:---|:---|
| REQ-NF-SEC-AUTHZ-001 | RBAC | Policy Engine | **미정의 권한 요청 100% 거부**<br>(Admin / User 2개 역할을 기본으로 제공하며, 역할별 권한 매트릭스를 코드로 관리) |
...
| ID | Category (범주) | Verification (검증) | Acceptance Criteria (인수/측정 기준) |
|:---|:---|:---|:---|
| REQ-NF-SEC-KEY-001 | KMS | Supabase/Cloud KMS 로그 | **Managed KMS 사용**<br>(DB 및 저장 데이터 암호화 키는 Supabase/Cloud에서 제공하는 관리형 KMS를 통해 안전하게 보관 및 관리되어야 한다.) |
| REQ-NF-SEC-KEY-003 | DEK 로테이션 | KMS | **로테이션 주기 준수 100%**<br>(Data Encryption Key 90일 로테이션을 준수하여야 한다.) |
...
| REQ-NF-SEC-VULN-002 | 의존성 스캔 | CI 로그 | **빌드당 스캔 실행률 100%**<br>(SCA(Software Composition Analysis) 매 CI 빌드 시 자동 실행 (GitHub Dependabot/Snyk 등)) |
| REQ-NF-SEC-VULN-004 | 침투 테스트 | 외부 전문업체 침투테스트 결과 보고서 | **연 1회 수행 + 결과 보고서**<br>(연 1회 침투 테스트를 수행하고, 발견 취약점에 대한 SLA 기반 해소 일정을 적용하여야 한다.) |
...
sequenceDiagram
    ...
    B-->>W: 매핑 데이터 반환 (≤ 10분, 실패율 < 0.5%)
    W-->>W: 클라이언트 측 PDF 렌더링 (html2pdf.js)
    W-->>P: 리포트 다운로드 완료
...
sequenceDiagram
    ...
    Note over E,B: REQ-FUNC-013: 실시간 연동, 유실률 = 0%
    alt 네트워크 온라인
        E->>B: POST /api/v1/edge/sync (Raw Payload + Raw SHA)
        B-->>B: Raw SHA-256 무결성 검증 (전송 위변조 검사)
        B-->>B: LLM 구조화 후 JSON 신규 SHA 생성
        B->>DB: 정규 JSON + Timestamp + Raw SHA + JSON SHA 체인 적재
        B-->>E: 동기화 성공 (유실률 = 0%)
    else 네트워크 오프라인
        E-->>O: ⚠️ 네트워크 연결 끊김 알림 (온라인 전용)
    end
```
...
sequenceDiagram
    ...
    participant AL as INSERT_ONLY_AUDIT_LOG
    participant DB as DB

    Note over Admin,B: REQ-FUNC-026: RBAC 권한 관리 (2단계)
    Admin->>W: 사용자 권한 변경 요청
    W->>B: 권한 변경 API 호출 (user_id, new_role)
    B->>RBAC: 역할 권한 정책 검증 (Admin/User)
    RBAC-->>B: 정책 적합성 확인
    B->>DB: RBAC 권한 즉시 반영
    B->>IM: 권한 변경 감사 이벤트 생성
    IM-->>IM: 이벤트 SHA-256 해시 생성
    IM->>AL: AUDIT_LOG 기록 (변경 전/후 상태)
    AL-->>IM: 기록 완료 (Insert-only)
    B-->>W: 권한 변경 성공 응답
    W-->>Admin: 변경 완료 표시

    Note over B,AL: REQ-FUNC-024: 타임스탬프 + SHA-256 무결성
    ...
    Note over AL: REQ-FUNC-025: Insert-only 보존 + 삭제 불가
    rect rgb(255, 245, 238)
        Note over Admin,AL: 감사 로그 불변성 검증
        Admin->>W: 감사 로그 삭제/수정 시도
        W->>B: DELETE/PATCH 요청
        B->>AL: 기록 수정 시도
        AL-->>B: ❌ 거부 응답 (DB Policy 위반)
        B-->>W: 삭제/수정 불가 오류 반환
...
erDiagram
    ...
    NC_CASE ||--o{ SUBMISSION_LOG : logs
    LEAN_DIAGNOSIS ||--o{ SUBMISSION_LOG : logs
    SITE ||--o{ INSERT_ONLY_AUDIT_LOG : "tracked by"
...
| storage_type | string(20) | 저장 유형: `INSERT_ONLY` 고정 | NOT NULL, DEFAULT `INSERT_ONLY` |
